...
public class Stream_06_Test {
    ...
    private long iterateSum(long n) {
        return Stream.iterate(0L, i -> i+1).limit(n).collect(Collectors.summingLong(i -> i));
    }
    ...
    private long parallelIterateSum(long n) {
        return Stream.iterate(0L, i -> i+1).limit(n).parallel().collect(Collectors.summingLong(i -> i));
    }
